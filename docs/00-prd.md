...
系统以 Redis 作为集中缓存存储，以 Caffeine 作为应用进程内本地缓存，以 rsyslog + Kafka 承载访问事件采集，以 etcd 承载热点 key 下发和失效事件广播。
...
TmcJedis.set/del/expire
  -> Redis 写操作
  -> 当前节点 CaffeineLocalCache invalidate
  -> InvalidationEvent 写入 etcd
  -> 其他 SDK 节点 watch
  -> 其他节点 CaffeineLocalCache invalidate
```
...
SDK 内部按有赞 Hermes-SDK 的思路收敛为三个模块：
...
| 模块 | 包 | 职责 |
|---|---|---|
| SDK 接口模块 | `cn.ayice.tmc.sdk` | 对业务侧暴露 TMC 客户端入口、配置、自动配置和客户端指标。 |
| 热点模块 | `cn.ayice.tmc.hotkey` | 维护热点 key 状态、本地缓存读写、热点有效期和本地缓存容量/过期策略。 |
| 通信模块 | `cn.ayice.tmc.communication` | 访问事件上报、rsyslog 写入、etcd 热点监听和失效广播等 I/O 通信能力。 |

核心对象：

- SDK 接口模块：`TmcClient`、`TmcProperties`、`TmcAutoConfiguration`、`TmcMetrics`。
- 热点模块：`HotKeyManager`、`CaffeineLocalCache`、`HotKeyProperties`、`LocalCacheProperties`。
- 通信模块：`AccessReporter`，后续按“失效上报、热点发现监听、热点失效监听”继续补具体实现类。

通信模块不为可预见只有一种实现的组件提前抽 `interface`；只有当同一角色确实存在多种运行态实现时，再引入接口。否则直接使用具体类。

该原则适用于所有模块。当前项目只支持 Jedis，不抽 Redis 客户端适配接口；`TmcJedis` 直接把 `jedis.get(key)` 读取动作传给 `TmcClient`。
...
4. `AccessReporter` 异步将事件写入 rsyslog。
5. `HotKeyManager` 判断 key 是否是热点 key。
6. 非热点 key 直接执行 Redis 回调。
7. 热点 key 查询 `CaffeineLocalCache`。
8. 本地命中时直接返回本地 value。

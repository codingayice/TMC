...
2. `TmcJedis.get(key)` 进入 `TmcClient.get(key)`。
3. SDK 回源时通过 `JedisRemoteCacheClient` 调用真实 `jedis.get(key)`。
...
| `cn.ayice.tmc.jedis.remote` | Jedis 到 SDK 远端缓存接口的适配 |

建议类：

```text
cn.ayice.tmc.jedis.TmcJedis
cn.ayice.tmc.jedis.remote.JedisRemoteCacheClient
```
...
### 5.1 `JedisRemoteCacheClient`

`JedisRemoteCacheClient` 实现 `tmc-sdk` 中的 `RemoteCacheClient`。
...
SDK 需要回源
  ...
  -> JedisRemoteCacheClient.get(key)
  -> jedis.get(key)
```
...
RemoteCacheClient remoteCacheClient = new JedisRemoteCacheClient(jedis);
TmcClient tmcClient = ...;
...
- `JedisRemoteCacheClient` 和 `TmcJedis` 可以共用同一个 `Jedis` 实例。
- 本阶段不封装连接池。
...
`jedis.get(key)` 异常由 `JedisRemoteCacheClient` 原样抛出。
...
```text
JedisRemoteCacheClientTest
TmcJedisTest
```

### 10.1 `JedisRemoteCacheClientTest`

覆盖：

1. `get(key)` 调用真实 `jedis.get(key)`。
2. Jedis 返回 `null` 时透传 `null`。
3. Jedis 抛异常时不吞掉。

测试方式：

- 使用 Mockito mock `Jedis`。
- 不需要启动 Redis。
...
1. `get(key)` 调用 `TmcClient.get(key)`。
2. `set` 返回 `OK` 时调用 `TmcClient.invalidate(key)`。
3. `set` 非 `OK` 时不调用 `invalidate`。
4. `del` 返回大于 `0` 时调用 `invalidate`。
5. `del` 返回 `0` 时不调用 `invalidate`。
6. `expire` 返回 `1` 时调用 `invalidate`。
7. `expire` 返回 `0` 时不调用 `invalidate`。
8. Jedis 写操作抛异常时不调用 `invalidate`。
...
2. 创建 `cn.ayice.tmc.jedis.remote` 包。
3. 实现 `JedisRemoteCacheClient`。
4. 编写 `JedisRemoteCacheClientTest`，验证回源行为。
5. 创建 `cn.ayice.tmc.jedis` 包。
6. 实现 `TmcJedis` 构造器。
7. 实现 `TmcJedis.get`。
8. 编写 `get` 转发测试。
9. 实现 `set` 和写后失效测试。
10. 实现 `del` 和写后失效测试。
11. 实现 `expire` 和写后失效测试。
12. 运行 `mvn -pl tmc-jedis -am test`。
13. 运行 `mvn test`。
...
- [ ] 存在 `JedisRemoteCacheClient`。
- [ ] `JedisRemoteCacheClient.get` 调用真实 `jedis.get`。
- [ ] 存在 `TmcJedis`。
- [ ] `TmcJedis.get` 调用 `TmcClient.get`。
